...
각 후보에는 출처, URL, 발행일, 수집일, 우선순위, 주제, 검토 이유, 검토 질문, 추천 관련 교재를 둡니다. `npm run check:feed-review`는 sourceId, URL, 날짜, 상태, 우선순위, 관련 교재 slug, `/industry` 중복 여부를 확인합니다.

현재 범위에서는 검토 후보를 자동으로 파일에 저장하거나 자동 게시하지 않습니다. 다음 단계는 source별 포함/제외 키워드 필터와 `/industry` 해설 노트 초안 승격 스크립트입니다.
