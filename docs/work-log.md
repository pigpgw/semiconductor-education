...
- 글 상세의 기초/중급/고급 인터랙션 데이터를 `lib/lesson-interactions.ts`로 분리해 컴포넌트는 렌더링만 담당하게 리팩토링했습니다.
...
- 기초/중급/고급 모드 데이터는 `lib/lesson-interactions.ts`로 분리됐지만 현재 DRAM/HBM/EUV 3편에 맞춘 정적 설명입니다. 새 글을 추가하면 slug별 데이터도 함께 추가해야 합니다.
- 학습 인터랙션 허브의 원칙 문구는 현재 핵심 글 3편 기준입니다. NAND/패키징 글을 추가하면 원칙 예시를 확장해야 합니다.
...
11. 기초/중급/고급 모드 데이터를 장기적으로 MDX frontmatter까지 올릴지, 현재처럼 `lib/lesson-interactions.ts`에 둘지 결정합니다.
